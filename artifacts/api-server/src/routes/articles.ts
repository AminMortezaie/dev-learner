...
async function serializeArticle(row: {
  id: number;
  createdAt: Date | null;
  [k: string]: unknown;
}) {
...
articlesRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json(await Promise.all(rows.map(serializeArticle)));
  }),
);
...
articlesRouter.post(
  "/articles/polish",
  asyncHandler(async (req, res) => {
    const { content } = polishSchema.parse(req.body);
    try {
      ...
      if (msg.includes("429")) {
        ...
        throw new HttpError(429, wait ? `AI rate limit reached. Try again in ${wait}.` : "AI rate limit reached. Please wait a minute and try again.");
      }
      if (msg.includes("413")) {
...
articlesRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json(await serializeArticle(row));
  }),
);
...
articlesRouter.delete(
  ...
  asyncHandler(async (req, res) => {
    const id = parseIntParam(req.params.id, "id");
    // Delete dependent quizzes first
